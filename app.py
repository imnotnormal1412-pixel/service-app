...
# ЛОГІКА ВИБОРУ ДЛЯ ПОСЛУГ (ІЗ РОЗДІЛЕННЯМ НА НАПРЯМИ)
if selected_category == "Послуги":
    # Визначаємо доступні напрями (поля) всередині послуг
    ...
    selected_field = st.selectbox("Оберіть напрям робіт:", available_fields)
    
    # Фільтруємо підкатегорії для обраного напряму
